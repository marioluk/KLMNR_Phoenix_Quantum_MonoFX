...
CONFIG_FILE = "PRO-THE5ERS-QM-PHOENIX-GITCOP-config-110725.json"
...
def parse_time(time_str: str) -> dt_time:
    """Converte una stringa oraria in oggetto time"""
    try:
        if time_str == "24:00":
            return dt_time(23, 59, 59, 999999)
        return datetime.strptime(time_str, "%H:%M").time()
    except ValueError:
        logger.warning(f"Formato orario non valido: {time_str}")
        return dt_time(0, 0)

def is_trading_hours(symbol: str, config: Dict) -> bool:
    """Verifica se il simbolo è negli orari di trading validi"""
    try:
        ...
        for time_range in trading_hours:
            try:
                ...
                logger.warning(f"Formato orario non valido per {symbol}: {time_range}")
                continue
                
        return False
    except Exception as e:
        logger.error(f"Errore controllo orari trading per {symbol}: {str(e)}", exc_info=True)
        return True
...
class QuantumEngine:
    ...
    def _calculate_spin_impl(self, ticks: List[Dict]) -> Tuple[float, float]:
        ...
        positive = sum(1 for t in ticks if t.get('direction', 0) > 0)
        negative = len(ticks) - positive
        total = len(ticks)
        
        raw_spin = (positive - negative) / total
        confidence = min(1.0, abs(positive - negative) / (total ** 0.5))  # Parentesi aggiunta
    ...
    def get_signal(self, symbol: str) -> Tuple[str, float]:
        ...
        buy_thresh = thresholds.get('buy_signal', 0.55) * volatility
        sell_thresh = thresholds.get('sell_signal', 0.45) / volatility

        # 6. Generazione segnale
        signal = "HOLD"
        if entropy > buy_thresh and spin > self.spin_threshold * confidence:
            signal = "BUY"
        elif entropy < sell_thresh and spin < -self.spin_threshold * confidence:
            signal = "SELL"
...
class QuantumRiskManager:
    ...
    def _apply_all_limits(self, symbol: str, price: float, signal: str, size: float) -> float:
        """Applica limiti con dimensione minima configurabile"""
        if size <= 0:
            return 0.0

        # Applica limite dimensione minima da config

        symbol_config = self._config_manager.get_risk_config(symbol)
        if not symbol_config or 'risk_management' not in symbol_config:
            logger.error(f"Configurazione rischio mancante per {symbol}")
            return 0.0
        # Dimensione minima da configurazione
        min_size = symbol_config['risk_management'].get('contract_size', 0.01)
        size = max(size, min_size)
        
        # Resto della logica esistente...
        size = np.clip(size, 
                      self.symbol_data[symbol]['volume_min'], 
                      self._get_config(symbol, 'max_size', 1.0))

        size = self._apply_position_risk_limit(symbol, price, size)
        size = self._apply_margin_limit(symbol, price, signal, size)
        return self._apply_equity_limit(symbol, size)
    ...
    def _basic_validate_levels(self, symbol: str, entry: float, sl: float, tp: float) -> bool:
        """Validazione base senza dipendere da trading_system"""
        symbol_info = mt5.symbol_info(symbol)
        if not symbol_info:
            return False
            
        pip_size = self.engine._get_pip_size(symbol)
        spread = (symbol_info.ask - symbol_info.bid) / pip_size
        min_distance = max(15, spread * 1.5)  # Min 15 pips o 1.5x spread
        
        return (abs(entry - sl) >= (min_distance * pip_size)) and \
               (abs(entry - tp) >= (min_distance * pip_size))

    def _basic_fallback_levels(self, symbol: str, position_type: int, price: float) -> Tuple[float, float]:
        """Fallback base senza dipendere da trading_system"""
        pip_size = self.engine._get_pip_size(symbol)
        sl_pips = 50  # Valori fissi di fallback
        tp_pips = 100
        
        if position_type == mt5.ORDER_TYPE_BUY:
            return (round(price - sl_pips * pip_size, 6),
    ...
    def _validate_basic_params(self, symbol: str, price: float) -> bool:
        """Valida i parametri fondamentali prima del trading"""
        if price <= 0:
            logger.error(f"Prezzo non valido per {symbol}: {price}")
            return False
        ...
        if not is_trading_hours(symbol, self.config):
            logger.debug(f"Simbolo {symbol} fuori orario di trading")
            return False
    ...
    def _apply_margin_limit(self, symbol: str, price: float, signal: str, size: float) -> float:
        """Applica il limite basato sul margine disponibile"""
        account = mt5.account_info()
        if not account:
            return 0.0

        order_type = mt5.ORDER_TYPE_BUY if signal == "BUY" else mt5.ORDER_TYPE_SELL
        required_margin = mt5.order_calc_margin(order_type, symbol, size, price)
        
        if required_margin and required_margin > account.margin_free * 0.6:
            adjusted_size = (account.margin_free * 0.6 * size) / required_margin
            adjusted_size = self._round_to_step(adjusted_size, symbol)
            logger.warning(f"Margine insufficiente, size ridotto a {adjusted_size:.2f}")
            return adjusted_size
            
        return size

        
    def _apply_equity_limit(self, symbol: str, size: float) -> float:
        """Applica il limite basato sull'equity"""
        account = mt5.account_info()
        if not account:
            return 0.0

        max_lots_per_10k = self._get_config(symbol, 'max_lots_per_10k', 1.0)
        max_size = (account.equity / 10000) * max_lots_per_10k
        
        return min(size, max_size)
        
        
    def _apply_position_risk_limit(self, symbol: str, price: float, size: float) -> float:
        """Applica il limite di rischio per posizione"""
        account = mt5.account_info()
        if not account:
            return 0.0
            
        position_risk = size * price * self.symbol_data[symbol]['contract_size']
        max_position_risk = account.equity * self._get_risk_percent(symbol) * 1.5
        
        if position_risk > max_position_risk:
            new_size = max_position_risk / (price * self.symbol_data[symbol]['contract_size'])
            logger.warning(f"Riduzione dimensione per {symbol} per superamento limite rischio")
            return self._round_to_step(new_size, symbol)
...
class QuantumTradingSystem:
    ...
    def _verify_connection(self) -> bool:
        ...
        if not mt5.initialize() or not mt5.terminal_info().connected:
            logger.warning("Connessione MT5 persa, tentativo di riconnessione..._verify_connection")
            mt5.shutdown()
            time.sleep(5)
    ...
    def _validate_tick(self, tick) -> bool:
        ...
        return spread_pips < max_spread * 1.5
    ...
    def _can_trade_symbol(self, symbol: str) -> bool:
        ...
        max_trades = self._config.get('risk_parameters', {}).get('max_daily_trades', 3)
        return self.trade_count[symbol] < max_trades
                                
                                
        
    def _execute_trade(self, symbol: str, signal: str, tick, price: float, size: float) -> bool:
        """Versione corretta per High Stakes Challenge con:
        1. Gestione SL/TP automatica con fallback
        2. Validazione completa
        3. Tracking delle operazioni
        """
        try:
            ...
            request = {
                ...
                "tp": tp,
                "deviation": 10,
                "magic": self.config.get('risk_parameters', {}).get('magic_number', 707251),
                "comment": "QTS-TRADE",
            ...
            if result.retcode != mt5.TRADE_RETCODE_DONE:
                logger.error(f"Errore esecuzione {symbol}: {result.comment} (code: {result.retcode})")
                return False
    ...
    def _execute_trade(self, symbol: str, signal: str, tick, price: float, size: float) -> bool:
        """Esegue il trade con gestione errori migliorata"""
        try:
            # Calcola SL/TP
            position_type = mt5.ORDER_TYPE_BUY if signal == "BUY" else mt5.ORDER_TYPE_SELL
            
            sl, tp = self.risk_manager.calculate_dynamic_levels(symbol, position_type, price)
            
            # Chiama calculate_dynamic_levels dal risk_manager
            
            
            
            # Se SL/TP non validi, esegui senza protezioni
            if sl == 0.0 and tp == 0.0:
                logger.warning(f"Esecuzione senza SL/TP per {symbol} a causa di errori di calcolo")
            ...
            logger.info(f"Trade eseguito {symbol} {size} lots a {price} | SL: {sl} | TP: {tp}")
            return True
            
        except Exception as e:
            logger.error(f"Errore durante esecuzione trade: {str(e)}")
            return False
    ...
    def _close_position(self, position):
        ...
        if (datetime.now() - position.time) < timedelta(seconds=min_duration):
    ...
    def _manage_single_position(self, position, current_price: float):
        """Gestisce singola posizione - Gestisce una singola posizione con il prezzo corrente"""
        if not position:
            return
            
        try:
            existing_pos = mt5.positions_get(ticket=position.ticket)
            if not existing_pos:
                return
                
            symbol = position.symbol
            ticks = list(self.engine.tick_buffer.get(symbol, []))
            
            if len(ticks) < self.engine.min_spin_samples:
                return

            spin, confidence = self.engine.calculate_spin(ticks[-self.engine.spin_window:])
            # se ho un buy e spin diventa minore del threshold e viceversa chiudo posizioni
            if ((position.type == mt5.ORDER_TYPE_BUY and spin < -self.engine.spin_threshold) or
                (position.type == mt5.ORDER_TYPE_SELL and spin > self.engine.spin_threshold)):
                
                logger.info(f"Chiusura posizione {position.ticket} per inversione trend")
                self._close_position(position)
                
        except Exception as e:
            logger.error(f"Errore gestione posizione {position.ticket}: {str(e)}")
    ...
    def _check_position_timeout(self, position):
        """Controlla timeout posizione con gestione tipi corretta"""
        try:
            # Accesso sicuro alla configurazione
            config = self.config.config if hasattr(self.config, 'config') else self.config
            max_hours = config.get('risk_parameters', {}).get('max_position_hours', 24)
            
            # Verifica tipi e conversione
            if hasattr(position, 'time'):
                if isinstance(position.time, (int, float)):  # Timestamp
                    position_time = datetime.fromtimestamp(position.time)
                elif isinstance(position.time, datetime):
                    position_time = position.time
                else:
                    logger.warning(f"Formato tempo non supportato per posizione {position.ticket}")
                    return
                    
                # Calcolo durata in ore
                duration_hours = (datetime.now() - position_time).total_seconds() / 3600
    ...
    def _get_tick_freshness(self, symbol: str) -> float:
        """Restituisce i secondi dall'ultimo tick valido"""
        tick = mt5.symbol_info_tick(symbol)
        if not tick:
            return float('inf')
        return (time.time() - (tick.time_msc / 1000))

    def _check_buffer_quality(self, symbol: str) -> bool:
        """Verifica la qualità del buffer tick"""
        buffer = self.engine.tick_buffer.get(symbol, [])
        if len(buffer) < 10:  # Minimo 10 tick nel buffer
            return False
            
        # Verifica che non ci siano gap temporali eccessivi
        time_diffs = [buffer[i]['time'] - buffer[i-1]['time'] for i in range(1, len(buffer))]
        return all(diff < 5 for diff in time_diffs)  # Max 5 secondi tra tick

   
    def _validate_features_config(self):
        """Versione più robusta che gestisce l'assenza della sezione features"""
        if 'features' not in self.config.config:
            # Se la sezione features non esiste, la crea con valori di default
            self.config.config['features'] = {
                'stop_loss': {'enable': False},
                'take_profit': {'enable': False},
                'trailing_stop': {'enable': False}
            }
            return
            
        required_features = ['stop_loss', 'take_profit', 'trailing_stop']
        for feature in required_features:
            if feature not in self.config.config['features']:
                logger.warning(f"Configurazione mancante per feature: {feature}")
                self.config.config['features'][feature] = {'enable': False}
