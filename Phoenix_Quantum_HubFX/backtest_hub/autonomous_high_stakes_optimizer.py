...
class AutonomousHighStakesOptimizer:
    ...
    def generate_config_for_broker(self, aggressiveness: str, broker: str) -> str:
        """
        Genera e salva una configurazione ottimizzata per un broker specifico,
        mappando i simboli secondo il broker richiesto.
        Il file viene salvato come config_autonomous_high_stakes_[aggressiveness]_[broker]_production_ready.json
        """
        logger.info(f"🔧 Generazione config per broker {broker} e livello {aggressiveness}")
        config = self.generate_optimized_config(aggressiveness)
        # Applica mapping simboli per broker
        symbols = {}
        for symbol, params in config.get('symbols', {}).items():
            mapped_symbol = self.map_symbol_for_broker(symbol, broker)
            symbols[mapped_symbol] = params
        config['symbols'] = symbols
        # Salva con nome specifico per broker
        return self.save_config(config, f"{aggressiveness}_{broker}")
    """
    ...
    - Algoritmo di trading phoenix_quantum_hubfx_program.py
    - Dati storici MT5
    ...
    def __init__(self, optimization_days=60, output_dir=None):
        ...
        # Default: output in Phoenix_Quantum_HubFX/config
        hubfx_config_dir = os.path.abspath(os.path.join(self.base_dir, '..', 'config'))
        self.output_dir = output_dir or hubfx_config_dir
    ...
    def validate_config_structure(self, config: dict) -> bool:
        """
        Valida la struttura della configurazione generata secondo gli standard HubFX.
        Restituisce True se valida, False altrimenti. (Stub: estendere con regole reali)
        """
        required_sections = ["quantum_params", "risk_parameters", "symbols"]
        for section in required_sections:
            if section not in config:
                logger.error(f"❌ Configurazione mancante sezione obbligatoria: {section}")
                return False
        # Placeholder: aggiungere controlli dettagliati su parametri/sottosezioni
        return True

    def map_symbol_for_broker(self, symbol: str, broker: str) -> str:
        """
        Mapping simboli cross-broker tramite config/broker_symbol_mapping.json.
        Se il mapping non esiste, ritorna il simbolo originale.
        """
        mapping_path = os.path.abspath(os.path.join(self.base_dir, '..', 'config', 'broker_symbol_mapping.json'))
        try:
            with open(mapping_path, 'r', encoding='utf-8') as f:
                mapping_data = json.load(f)
            broker_map = mapping_data.get('broker_symbol_mapping', {}).get(broker, {})
            # Cerca mapping per indici, forex, ecc. (qui solo indici per esempio)
            for asset_class in broker_map:
                if symbol in broker_map[asset_class]:
                    return broker_map[asset_class][symbol]
        except Exception as e:
            logger.warning(f"Mapping simboli non trovato o errore: {e}")
        return symbol
    ...
    def create_base_config_template(self) -> Dict:
        ...
        base_config = {
            ...
            "trading_algorithm": {
                "name": "phoenix_quantum_hubfx_program",
                "version": "2.0",
    ...
    def save_config(self, config: Dict, aggressiveness: str) -> str:
        ...
        Il parametro 'aggressiveness' può includere anche il nome broker (es: 'conservative_OANDA')
        """
        ...
        symbols = {}
        for symbol, params in config.get('symbols', {}).items():
            # Mapping simbolo cross-broker (placeholder, da integrare)
            mapped_symbol = self.map_symbol_for_broker(symbol, broker="GENERIC")
            ...
            comment = f"Override generato dinamicamente per {mapped_symbol} - score {params.get('optimization_score', 0):.2f}"
        ...
        # Validazione struttura config prima del salvataggio
        if not self.validate_config_structure(production_config):
            logger.error("❌ Configurazione NON valida secondo gli standard HubFX. Non salvata.")
            raise ValueError("Struttura config non valida per HubFX")
        try:
...
def main():
    ...
    while True:
        ...
        print("3. 🏦 Genera config per broker specifico (esempio reale)")
        print("4. 🏆 Auto-Best (Genera tutti, mantiene solo il migliore)")
        print("5. ⚙️ Configurazione avanzata")
        print("6. ✅ Test validazione configurazioni")
        print("7. ❌ Esci")
        ...
        try:
            if choice == "1":
                ...
                print("\n🏦 Esempio: Genera configurazione per broker specifico (configurabile da file)")
                # Carica broker disponibili da file di configurazione
                brokers_file = os.path.abspath(os.path.join(os.path.dirname(__file__), '..', 'config', 'brokers_available.json'))
                try:
                    with open(brokers_file, 'r', encoding='utf-8') as f:
                        brokers_data = json.load(f)
                    brokers_list = brokers_data.get('brokers', [])
                    if not brokers_list:
                        print("❌ Nessun broker disponibile in config/brokers_available.json")
                        return
                except Exception as e:
                    print(f"❌ Errore lettura brokers_available.json: {e}")
                    return
                # Mostra opzioni dinamicamente
                for idx, broker_name in enumerate(brokers_list, 1):
                    print(f"{idx}. {broker_name}")
                broker_choice = input(f"👉 Scegli broker (1-{len(brokers_list)}): ").strip()
                try:
                    broker_idx = int(broker_choice) - 1
                    broker = brokers_list[broker_idx]
                except (ValueError, IndexError):
                    print("❌ Scelta non valida, uso broker di default THE5ERS")
                    broker = "THE5ERS"
                print("\n🎯 Scegli livello aggressività:")
                print("1. 🟢 Conservative")
                print("2. 🟡 Moderate")
                ...
                optimizer = AutonomousHighStakesOptimizer()
                filepath = optimizer.generate_config_for_broker(aggressiveness, broker)
                print(f"✅ Configurazione {aggressiveness} per broker {broker} generata e salvata: {os.path.basename(filepath)}")
            elif choice == "4":
