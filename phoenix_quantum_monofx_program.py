...
try:
    ...
    np = None
def log_signal_tick(symbol, entropy, spin, confidence, timestamp, price, esito, motivo_blocco=None):
    log_path = os.path.join("logs", "signals_tick_log.csv")
    header = ['timestamp', 'symbol', 'entropy', 'spin', 'confidence', 'price', 'esito', 'motivo_blocco']
    row = [timestamp, symbol, round(entropy, 4), round(spin, 4), round(confidence, 4), price, esito, motivo_blocco or ""]
    try:
        ...
        logging.getLogger("phoenix_quantum").error(f"Errore scrittura log segnali: {e}")
        ...
        with _block_reason_counter_lock:
            ...
            if _block_reason_total % _block_reason_period == 0:
                try:
                    ...
                    logging.getLogger("phoenix_quantum").info(summary.strip())
                    _block_reason_counter.clear()
                except Exception as e:
                    logging.getLogger("phoenix_quantum").error(f"Errore logging riepilogo motivi di blocco: {e}")
    log_signal_tick(symbol, entropy, spin, confidence, timestamp, price, esito, motivo_blocco)
...
def main():
    ...
    engine = QuantumEngine(config_manager.config)
    def tick_acquisition_worker():
        while True:
            try:
                engine.main_tick_acquisition_loop()
            except Exception as e:
                logging.getLogger("phoenix_quantum").error(f"[THREAD] Errore nel ciclo acquisizione tick: {e}", exc_info=True)
            time.sleep(1)
...
class TradingMetrics:
    """Monitoraggio e calcolo delle metriche di performance trading"""
    def __init__(self):
        self.metrics = {
            'total_trades': 0,
            'win_trades': 0,
            ...
            'sharpe_ratio': 0.0,
            'profit_factor': 0.0,
            'symbol_stats': defaultdict(lambda: {'trades': 0, 'profit': 0.0, 'win': 0, 'loss': 0})
        }
    ...
    def update_trade(self, symbol: str, profit: float):
        ...
        if profit > 0:
            ...
            self.metrics['symbol_stats'][symbol]['win'] += 1
        else:
            self.metrics['loss_trades'] += 1
            self.metrics['symbol_stats'][symbol]['loss'] += 1
        self.metrics['symbol_stats'][symbol]['trades'] += 1
        self.metrics['symbol_stats'][symbol]['profit'] += profit
    ...
    def _calculate_metrics(self):
        ...
        self.metrics['win_rate'] = win_trades / total_trades if total_trades > 0 else 0.0
        self.metrics['avg_profit'] = np.mean(profits) if len(profits) > 0 else 0.0
        ...
        self.metrics['profit_factor'] = gross_profit / gross_loss if gross_loss > 0 else 0.0

    def _calculate_drawdown(self, profits: np.ndarray) -> float:
        """Calcola il massimo drawdown dalla serie di profitti"""
        ...
        self._drawdown_history.append(max_drawdown)
        return abs(max_drawdown)

    def _calculate_sharpe(self, profits: np.ndarray) -> float:
        """Calcola lo Sharpe Ratio (assume risk-free rate = 0)"""
        if len(profits) < 2:
            return 0.0
    ...
    def get_symbol_stats(self, symbol: str) -> dict:
        """Restituisce le statistiche per un simbolo"""
        return self.metrics['symbol_stats'][symbol]

    def log_performance_report(self):
        """Logga un report dettagliato delle metriche"""
        logger = logging.getLogger("phoenix_quantum")
        ...
        logger.info(
            ...
            f"Avg Profit: {summary['avg_profit']:.2f}\n"
