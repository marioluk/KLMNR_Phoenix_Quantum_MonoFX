...
class The5ersGraphicalDashboard:
    def __init__(self, config_file: str, log_file: str = None, use_mt5: bool = True):
        """
    ...
    def analyze_log_file(self):
        """Analizza tutto il file di log per dati storici"""
        if not os.path.exists(self.log_file):
            print(f"❌ File di log non trovato: {self.log_file}")
            return
    ...
    def analyze_log_line(self, line: str) -> Optional[dict]:
        """Analizza una riga di log e aggiorna i dati"""
        line = line.strip()
        if not line:
            return None
        ...
        # Heartbeat with quantum data (E=entropy, S=spin)
    ...
    def create_pnl_chart(self) -> Dict:
        ...
        if not self.pnl_history:
            # Placeholder con dati di esempio
            trace = go.Scatter(
    ...
    def create_drawdown_chart(self) -> Dict:
        ...
        return {
            'data': [trace, soft_limit_line, hard_limit_line],
            'layout': layout
        }
    ...
    def create_balance_chart(self) -> Dict:
        ...
        if not self.balance_history:
            ...
            balance_trace = go.Scatter(
                x=[datetime.now().strftime('%Y-%m-%d %H:%M:%S')],
                y=[5000],  # Balance iniziale The5ers
                mode='lines',
                name='Balance',
            ...
            return {
                'data': [balance_trace, equity_trace],
                'layout': layout
            }
    ...
    def create_symbols_chart(self) -> Dict:
        ...
        if not self.symbol_performance or all(self.symbol_performance[symbol]['trades'] == 0 for symbol in self.symbol_performance):
            # Placeholder con simboli configurati
